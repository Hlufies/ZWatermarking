...
class WatermarkDataset(Dataset):
    def __init__(self, train_dataset_arr, device):
        self.dataset = train_dataset_arr
        self.device = device

    def __getitem__(self, index):
        #   data, labels, pos, disc, zws, wm 
        return self.dataset[index]['data'].to(self.device), self.dataset[index]['labels'].to(self.device),\
            self.dataset[index]['pos_or_neg'].to(self.device), self.dataset[index]['disc'].to(self.device), \
            self.dataset[index]['zws'].to(self.device), self.dataset[index]['wm'].to(self.device)
